...
public class Login {
	...
	// Important - Before testing the TICKET table insert a row in Employee table containing
	// the respective UserId which will be inserted in the TICKET table.
	/*------------------------------------------------------------------------------------*/
	...
	public Login(){
		...
		try {
			...
			conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/approve-with","root","");
	...
	// Putting user data into HashMap
	 ...
	 // Retrieving All Data from HashMap for USER
		 ...
		 // Generating ticket for USER front-end Dashboard
		 public void create_ticket() {
			 ...
			 ticket_desc = scan.nextLine();
			 System.out.print("Enter Ticket Priority\n");
			 priority = scan.nextLine();
			 System.out.print("Enter Ticket Category\n");
			 ticket_cat = scan.nextLine();
             System.out.print("Enter watcher\n");
             watcher=scan.nextLine();
                         	
			
			// Cleaning the data if unstructured or if any unwanted character is present.
		        String sanTicketDesc = sanitizeInput(ticket_desc);
		        String sanTicketPriority = sanitizeInput(priority);
		        String sanTicketCat = sanitizeInput(ticket_cat);
		        String sanEMPId = sanitizeInput(uid);
		        String sanTicketWatcher = sanitizeInput(watcher);
		        
		        try{
		            PreparedStatement prepared_stmt = (PreparedStatement)conn.prepareStatement("INSERT INTO ticket VALUES(?,?,?,?,?,?,?,?,?,?,?)");
		            prepared_stmt.setString(1, sanEMPId);
		            prepared_stmt.setString(2, "Username");
		            prepared_stmt.setString(3, "");
		            prepared_stmt.setString(4, sanTicketPriority);
		            prepared_stmt.setString(5, sanTicketWatcher);
		            prepared_stmt.setString(6, "Open");
		            prepared_stmt.setString(7, sanTicketCat);
		            prepared_stmt.setString(8, sanTicketDesc);		
		            prepared_stmt.setTimestamp(9, sqlTime);
		            prepared_stmt.setNull(10, Types.NULL);
		            prepared_stmt.setNull(11, Types.NULL);
		            int exec = prepared_stmt.executeUpdate();
		            System.out.println(exec+" Ticket generated!");
		            conn.close();
		        }
		        catch(Exception e){
				    System.out.println("Error: "+e);
		 ...
		 // Withdraw ticket which changes the status of ticket to "Withdrawn" and ResolveDate to '0000-00-00 00:00:00'
		 public void withdraw_ticket()
			{
				...
				try
				{							
					if (uti.getStatus().equals("Withdrawn")) 
						{
				           	System.out.println("Ticket is already Withdrawn!");
						}
						
					else
						{  
					    	prepared_statement = (PreparedStatement) conn.prepareStatement("update ticket set TicketStatus='Withdrawn', ResolveDate='0000-00-00 00:00:00' where TicketID=?");
					    	prepared_statement.setString(1, tid);
	...
	public static void main(String args[]) throws Exception {
    	...
    	System.out.print("6. Generate New Ticket\n");
    	System.out.print("7. Withdraw ticket\n");
    	System.out.print("**********************************************************************\n");
    	int n = sc.nextInt();
    	switch(n) {
    	...
    	case 6 :  obj.create_ticket(); break;
    	case 7 :  obj.withdraw_ticket(); obj.user_put_data(); break;
    		}
    	}
	sc.close();
  }
