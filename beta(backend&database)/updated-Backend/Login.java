...
public class Login {
	...
	public Login(){
		...
		try {
			...
			conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/Helpdesk1","root","");
	...
	// Putting user data into HashMap
	 ...
	 // Retrieving All Data from HashMap for USER
		 ...
		 //withdraw ticket which changes the status of ticket to "resolved"
		 public void withdraw_ticket()
			{
				PreparedStatement prepared_statement;
				Scanner sc = new Scanner(System.in);
				System.out.println("Enter Ticket-ID");
				String tktIDTocheck = sc.nextLine();
				java.util.Date date=new java.util.Date();
				java.sql.Timestamp sqlTime=new java.sql.Timestamp(date.getTime());
				try
				{
					prepared_statement = (PreparedStatement) conn
					...
					if(resultset.next()) 
					{
						if (resultset != null) 
						{
							...
							{  
						    	 prepared_statement = (PreparedStatement) conn.prepareStatement("update ticket set TicketStatus='Resolved' where TicketID=?");
						    	 prepared_statement.setString(1, tktIDTocheck);
						         prepared_statement.executeUpdate();
						         prepared_statement = (PreparedStatement) conn.prepareStatement("update ticket set ResolveDate=? where TicketID=?");
						         prepared_statement.setTimestamp(1, sqlTime);
						         prepared_statement.setString(2, tktIDTocheck);
						         prepared_statement.executeUpdate();
	...
	public static void main(String args[]) throws Exception {
    	...
    	case 7 :  obj.withdraw_ticket(); break;
    		}
    	}
	sc.close();
  }
